...
from app.schemas.booking import BookingCreate, BookingUpdate, BookingResponse, BookingOptimizeRequest, BookingOptimizeResponse
...
def optimize_booking(request: BookingOptimizeRequest, db: Session = Depends(get_db)):
    ...
    optimal_room = find_optimal_room(db, request.start_time, end_time, request.required_capacity)
    if not optimal_room:
...
def get_available_slots(room_id: int, booking_date: date, db: Session = Depends(get_db)):
    """
